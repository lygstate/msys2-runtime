...
hook_or_detect_cygwin (const char *name, const void *fn, WORD& subsys, HANDLE h)
{
  ...
  for (PIMAGE_IMPORT_DESCRIPTOR pd = pdfirst; pd->FirstThunk; pd++)
    {
      char *name_to_compare = rva (PSTR, map ?: (char *) hm;
      bool not_msys2 = !ascii_strcasematch (name_to_compare, pd->Name - delta), "msys-2.0.dll");
      bool not_cygwin = !ascii_strcasematch (name_to_compare, pd->Name - delta), "cygwin1.dll");
